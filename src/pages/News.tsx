...
export default function News() {
  ...
  return (
    <div className="space-y-12 py-12" style={{ fontFamily: "'Inter', sans-serif" }}>
      <style>{`
        ...
        {FILTERS.map((cat) => {
          ...
          return (
            <button
              ...
              className="px-4 py-1.5 rounded-full text-xs font-bold tracking-wider transition-colors duration-200 flex items-center gap-1.5"
              style={{
        ...
        {filteredNews.length > 0 ? (
          filteredNews.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className="group rounded-3xl overflow-hidden shadow-sm hover:shadow-md transition-shadow duration-300 flex flex-col md:flex-row"
                style={{ backgroundColor: '#fff', border: `1px solid ${INK}0D` }}
              >
                {/* News Image (if exists) */}
               {/* News Image (if exists) */}
{item.image && (
  <div className="w-full md:w-[280px] lg:w-[320px] h-52 shrink-0 relative overflow-hidden" style={{ backgroundColor: `${INK}0D` }}>
    <img
      src={item.image}
      alt={item.content}
    ...
    />
                ...
                <div className="p-6 md:p-8 flex-grow flex flex-col justify-between space-y-6">
                  <div className="space-y-4">
                    ...
                    <h3 className="dsp text-xl md:text-2xl font-bold leading-snug" style={{ color: INK }}>
                      {item.content}
                    </h3>
